...
const NursePacientes: React.FC = () => {
  ...
  // Get today's date string in DD/MM/YYYY format to match mock data
  const todayStr = (() => {
    const now = new Date();
  ...
  const filtered = mockPatients.filter(p => {
    ...
    // Dependency grade filter
    if (grauFilter !== 0 && p.dependencyGrade !== grauFilter) return false;
    return true;
  });

  return (
    <AuthenticatedLayout requiredRole="Enfermeiro" pageTitle="Pacientes">
      {/* Filters */}
      <motion.div
      ...
      >
        ...
        <div className="flex flex-wrap gap-3 mb-3">
          <span className="text-sm text-[#666666] self-center mr-1">Ala:</span>
        ...
        <div className="flex flex-wrap gap-3">
          <span className="text-sm text-[#666666] self-center mr-1">Grau:</span>
          <button
            onClick={() => setGrauFilter(0)}
            className={`px-4 py-1.5 rounded-full text-xs font-semibold transition-all cursor-pointer ${
              grauFilter === 0 ? 'bg-[#003D5C] text-white' : 'bg-[#F8F9FA] text-[#666666] hover:bg-[#E5E5E5]'
            }`}
          >
            Todos
          </button>
          {[1, 2, 3].map(g => {
            ...
            return (
              <button
                key={g}
                onClick={() => setGrauFilter(g as GrauFilter)}
                className={`px-4 py-1.5 rounded-full text-xs font-semibold transition-all cursor-pointer ${
                  grauFilter === g
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            >
              ...
              <div className="flex items-start justify-between mb-3">
                <div>
                  ...
                  <p className="text-xs text-[#666666]">{patient.age} anos</p>
                </div>
                <span className="bg-[#003D5C]/10 text-[#003D5C] text-xs font-semibold px-2.5 py-1 rounded-md">
                  {patient.bedNumber}
                </span>
              </div>
              ...
              <div className="flex flex-wrap gap-1.5 mb-3">
                {patient.chronicDiseases.map(d => {
                  ...
                  return (
                    <span key={d} className={`${colors.bg} ${colors.text} text-[11px] font-medium px-2 py-0.5 rounded-full`}>
                      {d}
                    </span>
                  );
                })}
              ...
              {/* Dependency Grade Badge */}
              <div className="mb-3">
                <span className={`${gradeInfo.bg} ${gradeInfo.text} text-[11px] font-semibold px-2.5 py-1 rounded-full`}>
                  {gradeInfo.label} — {gradeInfo.tag}
                </span>
              </div>

              {/* Today's record indicator */}
              ...
              <div className="mt-auto flex flex-col gap-2">
                ...
                <motion.button
                  whileTap={{ scale: 0.98 }}
                  onClick={() => navigate(`/enfermeiro?pacienteId=${patient.id}`)}
        ...
        {filtered.length === 0 && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="col-span-3 bg-white rounded-2xl shadow-sm p-8 text-center"
          >
            <p className="text-[#666666]">Nenhum paciente encontrado com os filtros selecionados.</p>
          </motion.div>
        )}
      </div>
